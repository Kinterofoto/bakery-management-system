-- Remove fallback logic from audit triggers
-- Now we rely only on session context set by the application

-- 1. Update order_items_audit trigger function (remove fallback)
...
BEGIN
    ...
    -- Get related order_id (for reference only, not for fallback)
...
-- 2. Update order_item_deliveries_audit trigger function (remove fallback)
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE 'Audit triggers updated - fallback removed';
    RAISE NOTICE 'Now relying only on session context from application';
    RAISE NOTICE '';
END $$;
