...
import { AlertCircle, CheckCircle2, Package, AlertTriangle, Calendar, Truck } from "lucide-react"
  ...
  const handleCreateTransfer = async () => {
    try {
      ...
      toast({
        ...
        description: `Se creó la transferencia consolidada a ${pesajeWorkCenter?.name || 'PESAJE'} con ${consolidatedMaterials.length} materiales`,
      })
      await refetch()
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      ">
        <div className="p-4 border-b border-white/20 dark:border-white/10">
          <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            Materiales Consolidados para {pesajeWorkCenter?.name || 'PESAJE'}
          </h3>
        </div>
      ...
      <div className="flex justify-end">
        <Button
          ...
          disabled={isCreatingTransfer || consolidatedMaterials.length === 0}
          className="
            bg-blue-600
            text-white
            font-semibold
        ...
        >
          ...
          {isCreatingTransfer ? "Creando..." : "Crear Transferencia Consolidada"}
        </Button>
      </div>
    </div>
  )
