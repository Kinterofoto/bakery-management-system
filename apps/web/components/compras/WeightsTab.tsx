...
export function WeightsTab({ materialSuppliers: initialData, allMaterials, allSuppliers, onRefresh }: WeightsTabProps) {
  ...
  const assignmentsWithWeight = activeAssignments.filter(ms => !!ms.packaging_weight_grams && ms.packaging_weight_grams > 0).length
  const totalAssignments = activeAssignments.length

  const hasWeight = (ms: MaterialSupplierWithDetails): boolean => {
    return !!ms.packaging_weight_grams && ms.packaging_weight_grams > 0
  }
  ...
  const getCellValue = (assignmentId: string, ms: MaterialSupplierWithDetails): string => {
    ...
    return formatWeight(ms.packaging_weight_grams ?? null)
  }
  ...
  const saveCell = useCallback(async (assignmentId: string, ms: MaterialSupplierWithDetails, value: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({ packaging_weight_grams: newWeight })
        .eq("id", assignmentId)

      if (error) throw error

      setLocalData(prev => prev.map(item =>
        item.id === assignmentId ? { ...item, packaging_weight_grams: newWeight } : item
      ))
      ...
      toast({ title: "Error al guardar", description: "No se pudo actualizar el peso por paquete.", variant: "destructive" })
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white/70 dark:bg-black/50 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl shadow-lg shadow-black/5 p-6">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Asignaciones con peso</p>
          <p className="text-3xl font-semibold mt-2">
            <span className={assignmentsWithWeight === totalAssignments ? "text-green-600" : "text-orange-500"}>{assignmentsWithWeight}</span>
      ...
      <div className="bg-white/70 dark:bg-black/50 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl shadow-lg shadow-black/5 p-6">
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span>Peso por paquete (gr) — editable, se guarda automáticamente</span>
          </div>
        </div>
      </div>
                  ...
                  return (
                    <tr key={row.material.id} className="hover:bg-white/30 dark:hover:bg-white/5 transition-colors duration-150">
                      ...
                      {Array.from({ length: maxSuppliers }, (_, i) => {
                        ...
                        const missingWeight = !hasWeight(ms) && !editedCells.has(ms.id)

                        return (
                          <td key={i} className={`px-0 py-0 border-r border-gray-200/30 dark:border-white/10 last:border-r-0 ${missingWeight ? "bg-red-50/60 dark:bg-red-900/10" : ""} ${ms.is_preferred ? "bg-blue-50/30 dark:bg-blue-900/10" : ""}`}>
                            <div>
                              <div className={`px-2 py-1 border-b flex items-center justify-between gap-1 ${missingWeight ? "bg-red-100/60 dark:bg-red-900/20 border-red-200/40 dark:border-red-800/30" : "bg-gray-100/80 dark:bg-white/5 border-gray-200/40 dark:border-white/10"}`}>
                              ...
                              <div className="relative px-1 py-1">
                                <input
                                  ...
                                  className={`w-full text-center font-semibold text-base py-1.5 px-2 rounded-lg bg-transparent border border-transparent hover:border-gray-300/50 dark:hover:border-white/20 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 focus:bg-white dark:focus:bg-gray-800 outline-none transition-all duration-150 ${isSaving ? "text-blue-500" : missingWeight ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-white"} ${isSaved ? "text-green-600" : ""} ${!value ? "text-red-400 italic" : ""}`}
                                  placeholder="—"
                                />
                                {isSaving && (
