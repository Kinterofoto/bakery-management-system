...
export default function HistorialPage({ params }: Props) {
  ...
  const [selectedWorkCenter, setSelectedWorkCenter] = useState(workCenterId)
  const [dateRange, setDateRange] = useState<"week" | "month" | "all">("week")

  const workCenter = getWorkCenterById(selectedWorkCenter)

  // Filtrar turnos completados del centro de trabajo seleccionado
  const completedShifts = shifts.filter(shift =>
    shift.work_center_id === selectedWorkCenter && shift.status === "completed"
  ).sort((a, b) => {
  ...
  const formatBogotaDate = (dateString: string) => {
    // Supabase quita la Z, necesitamos agregarla para que JavaScript entienda que es UTC
    const utcString = dateString.endsWith('Z') ? dateString : dateString + 'Z'
    const utcDate = new Date(utcString)

    // Bogotá es UTC-5, restamos 5 horas
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Historial de Producción</h1>
            <p className="text-gray-600">{workCenter?.name || "Cargando..."}</p>
          </div>
        </div>
      </div>
