...
const SETTING_KEY = "production_wc_inventory_enabled"

export function useWcInventoryEnabled() {
  ...
  const fetchSetting = useCallback(async () => {
    try {
      ...
      console.error("Error reading production_wc_inventory_enabled:", err)
  ...
  const setEnabledRemote = useCallback(async (next: boolean) => {
    try {
      ...
      console.error("Error updating production_wc_inventory_enabled:", err)
...
export async function fetchWcInventoryEnabled(): Promise<boolean> {
  try {
    ...
    console.error("fetchWcInventoryEnabled fallback to true:", err)
    return true
  }
}
