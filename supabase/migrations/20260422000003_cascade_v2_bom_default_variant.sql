-- Cascade v2 functions: filter BOM joins to the default variant only.
-- BOM now has a variant_id column (see 20260422000001_bom_variants.sql). All
-- cascade / planning paths must read only rows belonging to the default
-- variant so purchasing + planning behavior is identical to the pre-variant
-- era. Pesaje picks variants elsewhere and does not call these functions.
--
-- This migration re-creates each function whose body joined bill_of_materials,
-- adding a JOIN to produccion.bom_variants on is_default. Function bodies are
-- otherwise byte-identical to their previous definitions in
-- 20260211000001_cascade_v2.sql and 20260216000001_cascade_v2_staff_affects_speed.sql.
                ...
                WHEN ps.cascade_source_id IS NOT NULL THEN
                    src.end_date + COALESCE(
                        (SELECT bom.tiempo_reposo_horas * interval '1 hour'
                         ...
                         JOIN produccion.bom_variants bv ON bv.id = bom.variant_id AND bv.is_default
